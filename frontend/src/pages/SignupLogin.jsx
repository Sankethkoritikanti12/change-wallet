...
export default function SignupLogin({ onSuccess }) {
  ...
  const [step, setStep] = useState(1); // 1 = details, 2 = pick card
  ...
  async function handleSignup() {
    ...
    const cleanPhone = phone.replace(/\D/g, '');
    try {
      const data = await api.registerCustomer(name, cleanPhone, email, address, city, state, zip);
      // Pass selected card theme along with customer data
  ...
  async function handleLogin(e) {
    ...
    const cleanPhone = phone.replace(/\D/g, '');
    try {
      const data = await api.lookupCustomer(cleanPhone);
  ...
  if (mode === 'signup' && step === 2) return (
    <div style={{ maxWidth: 400, margin: '0 auto', padding: '40px 16px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <button onClick={() => setStep(1)} style={{
        ...
        fontSize: 14, color: '#999', padding: 0, marginBottom: 20
      }}>
        ← Back
      </button>
      ...
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 14, color: '#999', margin: 0 }}>
          Choose a design for your Change Wallet card
        </p>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: 22 }}>{selectedCard.emoji}</p>
        </div>
      </div>
  ...
  return (
    <div style={{ maxWidth: 400, margin: '0 auto', padding: '40px 16px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 32 }}>
        ...
        <p style={{ fontSize: 14, color: '#999', margin: 0 }}>
          Keep your coin change — load it to a card instead
        </p>
      </div>
      ...
      <div style={{
        ...
        {['login', 'signup'].map(m => (
          <button key={m} onClick={() => { setMode(m); setError(''); setStep(1); }} style={{
            ...
            transition: 'all 0.2s', color: mode === m ? '#0F6E56' : '#999'
          }}>
      ...
      <form onSubmit={mode === 'login' ? handleLogin : handleNextStep}>
        ...
        {mode === 'signup' && (
          <>
            ...
            <div style={{
              background: '#f9f9f9', borderRadius: 14, padding: 16,
              marginBottom: 14, border: '1.5px solid #e5e5e5'
            }}>
              ...
              <p style={{ fontSize: 12, color: '#999', margin: '0 0 14px' }}>
                We'll mail your physical Change Wallet card here
              </p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10, marginBottom: 10 }}>
                <div>
                  ...
                  <input style={inputStyle} type="text" value={city}
                    ...
                    placeholder="Southfield" required />
                </div>
                <div>
      ...
      <p style={{ fontSize: 12, color: '#bbb', marginTop: 24, textAlign: 'center', lineHeight: 1.6 }}>
        Your phone number is your card. Give it at the register to get coin change added automatically.
      </p>
    </div>
  );
}